...
int main() {
	...
	for (int i = newChar.getRegion(); i < 4; i++) {
		...
		for (int i = 0; i < w1.getNum_Encounters(); i++) {
			...
			case 2:
				if (foundChest == 0) {
					std::cout << "You found a chest with 1000 coins inside!\n";
			...
			case 3:
				if (newChar.getRegion() == 0) {
					...
					std::cout << "A skeleton attacks!\n" << std::endl;
					combat combat(skeleton);
					combat.combat1(newChar);
					skeleton.mhealth = 100;
					newChar.charDef = newChar.getData(6);
					std::this_thread::sleep_for(std::chrono::seconds(5));
					system("cls");
				}
